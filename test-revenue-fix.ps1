...
foreach ($timestamp in $testDates) {
    ...
    try {
        $summaryUri = "$baseUrl/summary?period=day&date=$timestamp"
        $result = Invoke-RestMethod -Uri $summaryUri -Method Get
        ...
        if ($result.data.netRevenue -gt 0) {
            ...
            $detailsUri = "$baseUrl/details?period=day&date=$timestamp&limit=5"
            $detailResult = Invoke-RestMethod -Uri $detailsUri -Method Get
...
Write-Host "If no orders found, the fix is implemented correctly but there is no test data"
