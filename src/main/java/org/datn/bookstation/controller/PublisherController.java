...
public class PublisherController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Object>> add(@Valid @RequestBody PublisherRequest request, BindingResult bindingResult) {
    ...
    private void validatePublisherRequest(PublisherRequest request) {
        ...
        if (request.getPhoneNumber() != null && !request.getPhoneNumber().isEmpty()) {
            if (!request.getPhoneNumber().matches("^[0-9\\+\\-\\s\\(\\)]+$")) {
                throw new BusinessException("Số điện thoại chỉ được chứa số và các ký tự đặc biệt (+, -, space, (, ))", "INVALID_PHONE");
            }
        }
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Object>> update(@PathVariable Integer id, @Valid @RequestBody PublisherRequest request, BindingResult bindingResult) {
        // Kiểm tra validation errors
        if (bindingResult.hasErrors()) {
            List<ValidationErrorResponse> errors = new ArrayList<>();
            for (FieldError error : bindingResult.getFieldErrors()) {
        ...
        try {
            ...
            if (e.getMessage().contains("not found") || e.getMessage().contains("không tìm thấy")) {
                ApiResponse<Object> response = new ApiResponse<>(404, "Không tìm thấy nhà xuất bản với ID: " + id, null);
