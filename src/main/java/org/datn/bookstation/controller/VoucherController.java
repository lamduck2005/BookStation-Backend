...
public List<UserForVoucher> getUserByVuocherID(@PathVariable Integer voucherId) {
    ...
    public ResponseEntity<ApiResponse<List<org.datn.bookstation.dto.response.AvailableVoucherResponse>>> getAvailableVouchersForUser(@PathVariable Integer userId) {
        try {
            List<org.datn.bookstation.dto.response.AvailableVoucherResponse> availableVouchers = voucherService.getAvailableVouchersForUser(userId);
     ...
     * Tìm kiếm voucher theo mã hoặc tên để staff có thể tra cứu khi khách hàng đưa voucher
     */
    @GetMapping("/search")
