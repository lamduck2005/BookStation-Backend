...
public interface RewardRepository extends JpaRepository<Reward, Integer> {
    ...
    @Query("SELECT r FROM Reward r WHERE r.campaign.id = :campaignId AND r.status = 1 AND r.quantityRemaining > 0 ORDER BY r.probability DESC")
    ...
    // Đếm tổng số phần thưởng đã phát
    @Query("SELECT SUM(r.quantityTotal - r.quantityRemaining) FROM Reward r WHERE r.campaign.id = :campaignId")
